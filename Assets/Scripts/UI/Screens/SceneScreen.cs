...
{
    /// <summary>
    /// "Cave-heaven roster" - the qdao-themed selection screen. Same data
    /// model as <see cref="ServerSelectScreen"/> (live zone list from the
    /// Java gateway) but uses the SceneScreen FUI layout with the side
    /// scroll arrows and full-bleed backdrop.
    ///
    /// Visuals come entirely from the qdao FairyGUI package; this class
    /// binds named children and drives the two GLists (left filter tabs
    /// + center zone rows) from <see cref="SessionModel"/>.
    /// </summary>
    public sealed class SceneScreen : IScreen
    {
        private const float ScrollStepPixels = 220f; // ~ 2 rows per click
        ...
        public GComponent Build(AppBootstrap app)
        {
            ...
            _tabList     = Theme.Find<GList>(_root,      Theme.UiId.SceneTabList);
            _zoneList    = Theme.Find<GList>(_root,      Theme.UiId.SceneList);
            _statusLabel = Theme.Find<GTextField>(_root, Theme.UiId.SceneStatus);
            _confirmBtn  = Theme.Find<GButton>(_root,    Theme.UiId.SceneConfirmBtn);

            _zoneScroll = _zoneList != null ? _zoneList.scrollPane : null;

            // Hide static placeholder shortcut tabs (n6/n7/n8) in SceneScreen.xml.
            // The real left-column tab strip is the GList.
            HideIfPresent(Theme.UiId.SceneTabRecent);
            HideIfPresent(Theme.UiId.SceneTabRecommend);
            HideIfPresent(Theme.UiId.SceneTabAll);
            ...
            BindButton(Theme.UiId.SceneRefreshBtn,    () => _app.Run(LoadZones()));
        ...
        {
            if (_statusLabel == null) yield break;
            _loading = true;
        ...
        private void Rebind()
        {
            ...
            if (_visibleZoneIndices.Count == 0 && !_loading && _statusLabel != null)
        ...
        {
            if (_zoneScroll == null) return;
            _zoneScroll.posY = Mathf.Max(0f, _zoneScroll.posY - ScrollStepPixels);
        }

        private void ScrollDown()
        {
            if (_zoneScroll == null) return;
            float maxY = Mathf.Max(0f, _zoneScroll.contentHeight - _zoneScroll.viewHeight);
            _zoneScroll.posY = Mathf.Min(maxY, _zoneScroll.posY + ScrollStepPixels);
        }

        // ── Helpers ──────────────────────────────────────────────────

        private void BindButton(string childName, Action handler)
        {
        ...
        private void HideIfPresent(string childName)
        {
            var go = _root.GetChild(childName);
